...
def path_to_cache(dm : AffectNetDataModule):
    data_path = Path(dm.output_dir) / "cache" / "arrays" / "emotion"
    return data_path


def create_data_array(dm : AffectNetDataModule, emotion_feature="emo_feat_2", force_cache_load=False):
...
def main():
    ...
    dm = AffectNetDataModule(str(root_path), str(output_path), processed_subfolder=subfolder)
    dm.prepare_data()
