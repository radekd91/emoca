...
def main(args):
    ...
    logger = WandbLogger("Coma", experiment_name, config['InputOutput']['output_dir'], id=experiment_name)
    logger.add_config(config)
    training_loop(model, optimizer, lr_scheduler, train_loader, test_loader, start_epoch, total_epochs, device, config, logger)


def training_loop(model, optimizer, lr_scheduler,
                  train_loader, test_loader, start_epoch, total_epochs, device, config, logger):
    print('Initializing parameters')
    template_file_path = config['InputOutput']['template_fname']
    template_mesh = Mesh(filename=template_file_path)

    output_dir = config['InputOutput']['output_dir']
