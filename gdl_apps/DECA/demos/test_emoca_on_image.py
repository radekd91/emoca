from gdl_apps.DECA.utils.load import load_model
...
def save_obj(deca, filename, outputdict):
    '''
    ...
    vertices = outputdict['verts'][i].detach().cpu().numpy()
    faces = deca.deca.render.faces[0].detach().cpu().numpy()
    texture = util.tensor2image(outputdict['uv_texture_gt'][i])
    uvcoords = deca.deca.render.raw_uvcoords[0].detach().cpu().numpy()
    uvfaces = deca.deca.render.uvfaces[0].detach().cpu().numpy()
    # save coarse mesh, with texture and normal map
    normal_map = util.tensor2image(outputdict['uv_detail_normals'][i] * 0.5 + 0.5)
    ...
    normals = outputdict['normals'][i].detach().cpu().numpy()
    displacement_map = outputdict['displacement_map'][i].detach().cpu().numpy().squeeze()
