...
from gdl_apps.DECA.training.test_and_finetune_deca  import prepare_data
import torch
...
def main():
    ...
    dm, cfg = load_data(path_to_models, run_name, stage, relative_to_path, replace_root_path)
    image_index = 390*4

    dl = dm.train_dataloader()

    process = psutil.Process(os.getpid())
    checkpoint = locate_checkpoint(cfg, replace_root_path, relative_to_path, "best")
    deca = DecaModule.load_from_checkpoint(checkpoint,
                                           model_params=cfg.model,
                                           learning_params=cfg.learning,
                                           inout_params=cfg.inout,
                                           stage_name=""
    ...
    for batch_idx, batch in enumerate(auto.tqdm(dl)):
        ...
        for key in batch.keys():
            if isinstance(batch[key], torch.Tensor):
                batch[key] = batch[key].cuda()[:1,:1,...]


        loss = deca.training_step(batch, batch_idx)
        if batch_idx % 100 == 0:
