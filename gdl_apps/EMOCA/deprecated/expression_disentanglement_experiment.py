...
from gdl.utils.lightning_logging import  _log_wandb_image
...
def main():
    ...
    if len(sys.argv) <= 2:
        ...
        # EMOCA on EMOCA dataset, no ring, emotion loss
    ...
    if not use_original_deca:
        ...
        conf["coarse"].inout.full_run_dir = str(Path(conf["coarse"].inout.full_run_dir).parent.parent / "Original_DECA" / "coarse")
        conf["detail"].inout.full_run_dir = str(Path(conf["detail"].inout.full_run_dir).parent.parent / "Original_DECA" / "detail")
    ...
    if use_original_deca:
        ...
        print("Loading original EMOCA checkpoint")
